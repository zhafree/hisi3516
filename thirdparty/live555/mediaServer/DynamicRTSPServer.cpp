...
::lookupServerMediaSession(char const* streamName, Boolean isFirstLookupInSession) {
  ...
  if (!fileExists && strcmp(extension, ".live")) {
    if (smsExists) {
...
//#define INTERLEAVE_ADUS 1
    ...
    // Assumed to be a H.264 Live Video:
    NEW_SMS("H.264 Live Video");
    OutPacketBuffer::maxSize = 2000000; // allow for some possibly large H.264 frames
    sms->addSubsession(H264VideoLiveServerMediaSubsession::createNew(env, reuseSource));
    sms->addSubsession(ADTSAudioLiveServerMediaSubsession::createNew(env, reuseSource));
  }
